...
class BluetoothTerminal {
     ...
     */
    constructor(serviceUuid = '5e87bd74-b350-4a6a-ae36-bb33e30499af', characteristicUuid = '5e87bd74-b350-4a6a-ae36-bb33e30499af',
        receiveSeparator = '\n', sendSeparator = '\n') {
    ...
    /**
     * Connect to a specific service on the Bluetooth device.
     * @param {string} serviceUuid - The service UUID to connect to
     * @return {Promise} - Promise resolving to the service object
     */
    connectToService(serviceUuid) {
      if (!this._device) {
      ...
      return this._device.gatt.connect()
          ...
          .then(service => {
              this._log('Connected to service:', serviceUuid);
              return service;
          })
          .catch(error => {
              this._log('Failed to connect to service:', error);
              throw error;
          });
  }
   ...
   */
  writeToCharacteristic(serviceUuid, characteristicUuid, data) {
      return this.connectToService(serviceUuid)
          .then(service => service.getCharacteristic(characteristicUuid))
          .then(characteristic => characteristic.writeValue(data))
          .then(() => this._log('Data written to characteristic:', characteristicUuid))
          .catch(error => {
              this._log('Failed to write to characteristic:', error);
              throw error;
          });
  }
   ...
   * @param {ArrayBuffer} firmwareData - The firmware data to upload
   * @param {string} fileCharacteristicUuid - The characteristic UUID for file transfer
   * @param {number} chunkSize - The size of each data chunk
   * @return {Promise} - Promise that resolves when the firmware is fully uploaded
   */
  /**
 * Upload firmware to the connected device.
 * @param {ArrayBuffer} firmwareData - The firmware data to upload
 ...
 * @return {Promise} - Promise that resolves when the firmware is fully uploaded
 */
uploadFirmware(firmwareData, fileCharacteristicUuid, chunkSize = 512) {
  let totalSize = firmwareData.byteLength;
  let currentPosition = 0;

  const sendChunkRecursive = async (position) => {
      if (position >= totalSize) {
          return;
      }

      let isLastChunk = position + chunkSize >= totalSize;
      let currentChunkSize = isLastChunk ? totalSize - position : chunkSize;
      let chunk = firmwareData.slice(position, position + currentChunkSize);

      await this.writeToCharacteristic(this._serviceUuid, fileCharacteristicUuid, chunk);
      currentPosition += currentChunkSize;

      // 打印上传进度
      let progressPercent = (currentPosition / totalSize * 100).toFixed(2);
      console.log(`Firmware upload progress: ${progressPercent}%`);
  ...
  return sendChunkRecursive(currentPosition);
}
